...
#tau_publisher = [rospy.Publisher('joint_tau_{i}', Float64, queue_size=10) for i in range(6)]
tau_publisher = rospy.Publisher('joint_tau', Float64, queue_size=10)

while not rospy.is_shutdown():
  for i in range (6):
    tau_msg = Float64()
    tau_msg.data=z1.armState.tau[i]
    tau_publisher.publish(tau_msg) # unlock
    #tau_publisher.publish(z1.armState.tau[0])
    #tau_publisher[i].publish(tau_msg)

  time.sleep(0.1)
  #tau_publisher.publish(z1.armState.tau[0]) # unlock
  #time.sleep(z1.dt) # unlock
